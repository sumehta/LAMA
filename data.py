    ...
    def build_vocabulary(self):
        if not self._from_emb:
            ...
            try:
                ...
                # sw = stopwords.words("english")  # lot of negative words in stopwords which need to be removed
...
class Corpus( object ):
    ...
    def tokenize(self, data_dir, file):
        ...
        data = np.zeros((len(df), self.args.max_word), dtype=float)

        labels = np.zeros((len(df)), dtype=float)
        for idx, record in df.iterrows():
            labels[ idx ] = int( record['label'] )
            record['text'] = record['text'].replace('<s>', ' ')
            words = [w for w in record['text'].split(' ')]

            for i, word in enumerate(words):
                if i<self.args.max_word:
